...
const { ChannelSend } = require('../../lootbox/utils/embed')
...
const Spam = async (message, client, canal) => {
    ...
    try {
        let existSpawn = await Spawn.findOne({ where: { Guild: message.guild.id, Channel: canal } })
        if (existSpawn) {
            try {
                await Spawn.destroy({where: { Guild: message.guild.id, Channel: canal }})
            } catch (error) {
                console.error(error)
            }
        }
    ...
    return ChannelSend(message, client, canal, '', `A **${pokemon}**${disc} has appeared ${TranslatePlace(place)}. Use some poke ball before he manages to escape or someone anticipates you.\n\n**Level**: ${level}\n**HP**: ${psactual}/${psmax}\n**Status**: ${TranslateStatus(status)}`, pokeimg)
}
